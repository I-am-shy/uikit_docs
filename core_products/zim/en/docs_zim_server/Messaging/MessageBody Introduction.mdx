...
| Width | Number | Yes（Only if the first frame of the video is required） | Image width in pixels（px). If Thumbnail is set, this value cannot be 0. |
| Height | Number | Yes（Only if the first frame of the video is required） | Image height in pixels（px). If Thumbnail is set, this value cannot be 0. |
