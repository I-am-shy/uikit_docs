...
sequenceDiagram
    ...
    %% UserA initial flow
    note over ClientA,ClientB: UserA joins
    ClientA->>ZEGO Express SDK: UserA creates engine <br/> [new ZegoExpressEngine]
    ClientA->>ZEGO Express SDK: UserA logs in to room <br/> [loginRoom]
    ClientA->>ZEGO Express SDK: Set event notification callback <br/> [on]
    ClientA->>ZEGO Express SDK: UserA creates stream (Capture) <br/> [createZegoStream]
    ClientA->>ZEGO Express SDK: UserA starts publishing stream <br/> [startPublishingStream]
    ClientA->>ZEGO Express SDK: UserA starts playing stream <br/> [startPlayingStream]

    %% UserB joins stage
    note over ClientA,ClientB: UserB joins
    ClientB->>ZEGO Express SDK: UserB creates engine <br/> [new ZegoExpressEngine]
    ClientB->>ZEGO Express SDK: UserB logs in to room <br/> [loginRoom]
    ...
    ClientB->>ZEGO Express SDK: UserB creates stream (Capture) <br/> [createZegoStream]
    ClientB->>ZEGO Express SDK: UserB starts publishing stream <br/> [startPublishingStream]
    ZEGO Express SDK-->>ClientA: Notify A of stream update in room <br/> [roomStreamUpdate]
    ClientB->>ZEGO Express SDK: UserB starts playing stream <br/> [startPlayingStream]

    %% UserA exits stage
    note over ClientA,ClientB: UserA exits
    ClientA->>ZEGO Express SDK: UserA stops publishing stream <br/> [stopPublishingStream]
    ClientA->>ZEGO Express SDK: UserA destroys stream (Stop capture) <br/> [destroyStream]
    ZEGO Express SDK-->>ClientB: Notify B of stream update in room <br/> [roomStreamUpdate]
    ClientA->>ZEGO Express SDK: UserA stops playing stream <br/> [stopPlayingStream]
    ClientA->>ZEGO Express SDK: UserA logs out of room <br/> [logoutRoom]
    ZEGO Express SDK-->>ClientB: Notify B of user update in room <br/> [roomUserUpdate]

    %% UserB exits stage
    note over ClientA,ClientB: UserB exits
    ClientB->>ZEGO Express SDK: UserB stops publishing stream <br/> [stopPublishingStream]
    ClientB->>ZEGO Express SDK: UserB destroys stream (Stop capture) <br/> [destroyStream]
    ClientB->>ZEGO Express SDK: UserB stops playing stream <br/> [stopPlayingStream]
    ClientB->>ZEGO Express SDK: UserB logs out of room <br/> [logoutRoom]
```
{/*
...
sequenceDiagram
    ...
    App->>ZEGO Express SDK: Create stream (Capture) <br/> [createPusher]

    App->>ZEGO Express SDK: Publish stream <br/> [getPusherInstance().start()]
    ZEGO Express SDK-->>App: Publish state update/Quality callback <br/> [publisherStateUpdate / publishQualityUpdate]

    App->>ZEGO Express SDK: Play stream <br/> [getPlayerInstance().play()]
    ZEGO Express SDK-->>App: Play state update/Quality callback <br/> [playerStateUpdate / playQualityUpdate]

    App->>ZEGO Express SDK: Stop publishing stream <br/> [getPusherInstance().stop()]
    ZEGO Express SDK-->>App: Publish state update callback <br/> [publisherStateUpdate]

    App->>ZEGO Express SDK: Stop playing stream <br/> [getPlayerInstance().stop()]
