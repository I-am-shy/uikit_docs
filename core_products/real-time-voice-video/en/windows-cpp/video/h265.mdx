...
In this scenario, after the stream mixing service receives the streams from the host and co-hosting guests through ZEGO real-time audio and video cloud, it directly outputs one H.265 mixed stream and one H.264 mixed stream, and pushes both streams to the CDN. The audience can choose to pull H.265 streams or H.264 streams from the CDN according to whether their terminal devices support H.265 video decoding.
...
In this scenario, after the stream mixing service receives the streams from the host and co-hosting guests through ZEGO real-time audio and video cloud, it directly outputs one H.265 mixed stream and pushes this mixed stream to the CDN. Through the CDN's transcoding capability, the audience can choose to pull H.265 streams or H.264 streams from the CDN according to whether their terminal devices support H.265 video decoding.
...
- The host forwards to the CDN through ZEGO real-time audio and video cloud. Through the CDN's transcoding capability, the audience can choose to pull H.265 streams or H.264 streams from the CDN according to whether their terminal devices support H.265 video decoding.
- When the host pushes H.265 streams, co-hosting guests pull streams directly from ZEGO real-time audio and video cloud for real-time interaction. In this case, it is required that the co-hosting guests' terminal devices support H.265 video decoding capability.
...
<Step>
Call the [addPublishCdnUrl](@addPublishCdnUrl) interface to add the URL address for forwarding the audio and video streams from ZEGO real-time audio and video cloud to the CDN.
</Step>
<Step>
...
- **Co-hosting Guest Side**
  ...
  2. The co-hosting guest obtains the encoding format of this stream from the app's business server.
    - If it is H.264 format, the co-hosting guest can call the [startPlayingStream](@startPlayingStream) interface to pull the host's stream directly from ZEGO real-time audio and video cloud.
    - If it is H.265 format, they need to first call the [isVideoDecoderSupported](@isVideoDecoderSupported) interface to query whether their own device supports H.265 decoding format.
        - If supported, call the [startPlayingStream](@startPlayingStream) interface to pull H.265 streams from ZEGO real-time audio and video cloud.
