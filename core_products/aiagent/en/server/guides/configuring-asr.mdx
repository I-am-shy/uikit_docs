...
- 8k_zh_large: Chinese phone scenario dedicated large model engine [Large Model Version]. The current model supports Chinese, Shanghai, Sichuan, Wuhan, Guiyang, Kunming, Xi'an, Zhengzhou, Taiyuan, Lanzhou, Yinchuan, Xining, Nanjing, Hefei, Nanchang, Changsha, Suzhou, Hangzhou, Jinan, Tianjin, Shijiazhuang, Heilongjiang, Jilin, Liaoning, Minnan, Hakka, Cantonese, Nanning dialect recognition.

**Non-phone scenarios:**
- 16k_zh_en: Chinese-English-Cantonese+9 dialect large model engine [Large Model Version]. The current model supports Chinese, English, Cantonese, Sichuan, Shaanxi, Henan, Shanghai, Hunan, Hubei, Anhui, Minnan and Chaoshan dialect recognition;
- 16k_zh_large: Putonghua-English large model engine [Large Model Version]. The current model supports Chinese, English and 27 dialect recognition;
- 16k_multi_lang: Multi-language large model engine [Large Model Version]. The current model supports English, Japanese, Korean, Arabic, Filipino, French, Hindi, Indonesian, Malay, Portuguese, Spanish, Thai, Turkish, Vietnamese, German;
...
Set the model through the `model` parameter, the model value is `qwen3-asr-flash-realtime`.

#### Set Language

Set the language through the `language` parameter.
...
Set the model through the `model_name` parameter, the model value is `bigmodel`.
