...
| Add Singapore node | Added a Singapore cluster with default configuration for Singapore node ASR, LLM, TTS and other services. Suitable for overseas customers to reduce latency and deploy in multiple regions. Among them, zego_test account supports Singapore node qwen-3.5-flash and other models, and can configure self-procured overseas Gemini, OpenAI and other LLM providers; supports Singapore node MiniMax-TTS, for details please refer to [Configuring TTS](/aiagent-server/guides/configuring-tts); digital human service also supports Singapore node. | - |
